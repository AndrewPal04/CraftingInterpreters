...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
    ...
    private void endScope() {
        ...
        for (Map.Entry<String, VariableInfo> entry : scope.entrySet()) {
            if (entry.getValue().defined && !entry.getValue().used) {
                System.err.println("Warning: Local variable '" + entry.getKey() + "' is declared but never used.");
            }
        }
    }
    ...
    public Void visitClassStmt(Stmt.Class stmt) {
        ...
        if (stmt.superclass != null) {
            ...
            beginScope();
            scopes.peek().put("super", new VariableInfo(true, true, 0));
        }
        beginScope();
        scopes.peek().put("this", new VariableInfo(true, true, 0));
